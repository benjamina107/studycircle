...
for (const hardening of [false, true]) test(`chat file SQL isolates classes, verification, and identity (hardening=${hardening})`, async () => {
  ...
  try {
    ...
    for (const file of ['202609050001_initial.sql', '202609050002_domain.sql', '202609050003_meetups.sql', '202609050004_class_files.sql']) await db.exec(await readFile(new URL(`../supabase/migrations/${file}`, import.meta.url), 'utf8'));
    ...
    await db.exec(await readFile(new URL('../supabase/migrations/202609050005_chat_files.sql', import.meta.url), 'utf8'));
