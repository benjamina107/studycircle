...
export default function AuthForm({ mode }: { mode: "login" | "signup" | "resend" }) {
  ...
  return <form method="post" onSubmit={submit} className="auth-form" aria-busy={pending}>
    <div className="auth-form-heading"><h2>{signup ? "Create an account" : mode === "login" ? "Log in" : "Request a new code"}</h2><p>{signup ? "Use your Cal Poly email. You’ll need to verify it before signing in." : mode === "login" ? "Enter your Cal Poly email and password." : "Enter the email you used to sign up."}</p></div>
    ...
    <button disabled={pending} className="auth-submit">{pending ? "Please wait…" : signup ? "Create account" : mode === "login" ? "Log in" : "Send confirmation code"}</button>
