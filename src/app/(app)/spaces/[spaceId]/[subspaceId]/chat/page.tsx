...
export default async function ChatPage({ params, searchParams }: { params: Promise<{spaceId:string;subspaceId:string}>; searchParams: Promise<{channel?:string|string[]}> }) {
  ...
  const now = Date.now();
  return <main><ClassWorkspace key={subspaceId + channel} subspace={subspaceId} userId={user.id} initialChannel={channel} classLabel={group.spaces.courses.code + " · " + group.professors.name} invites={(meetups.data || []).map(item => ({id:item.id,title:item.title,when:meetupTime(item.starts_at),location:item.location_name,hosting:item.creator_id === user.id,started:Date.parse(item.starts_at) <= now,joined:!!attendance.data?.some(row => row.meetup_id === item.id)}))} /></main>;
}
