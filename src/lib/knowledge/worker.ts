...
export async function answerQuestion(q: Question) {
 ...
 const history=checked(await db.from('study_messages').select('role,body').eq('subspace_id',q.subspace_id).eq('channel',q.channel).lt('created_at',q.created_at).order('created_at',{ascending:false}).limit(6)) || [];
 const searchText=[...history.slice(0,2).reverse().map(m=>m.body.slice(0,800)),q.body].join('\n');
 ...
 if(!selected.length) {
  checked(await db.rpc('kb_finish_question',{question:q.id,token:q.lease_token,answer:{body:'There are no ready notes in this class yet. Upload your material in Shared notes, wait for Ready, then ask me again.',payload:{cards:[],sources:[]}}}));return;
 }
 const uploads=checked(await db.from('kb_uploads').select('id,asset_id,file_name,description').in('asset_id',[...new Set(selected.map(p=>p.asset_id))]).order('created_at').limit(500)) || [];
