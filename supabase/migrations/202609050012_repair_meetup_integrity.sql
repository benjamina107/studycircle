-- Repair projects where version 003 recorded chat hardening instead of meetup integrity.
-- Safe after the original meetup migration too; preserve applied history.
begin;
...
create index if not exists meetups_subspace_starts_idx on public.meetups(subspace_id, starts_at);
create index if not exists meetups_creator_created_idx on public.meetups(creator_id, created_at);
...
create table if not exists public.meetup_daily_post_limits (
...
create or replace function public.validate_meetup()
...
create or replace function public.enforce_meetup_post_limit()
returns trigger language plpgsql security definer set search_path = '' as $$
...
create or replace function public.add_meetup_creator_as_attendee()
returns trigger language plpgsql security definer set search_path = '' as $$
begin
...
drop trigger if exists validate_meetup on public.meetups;
...
drop trigger if exists z_enforce_meetup_post_limit on public.meetups;
...
drop trigger if exists add_meetup_creator_as_attendee on public.meetups;
