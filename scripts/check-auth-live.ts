...
async function main(){
 ...
 try{
  ...
  id=link.data.user.id;const token=link.data.properties.hashed_token;
  ...
  const verified=await post('verify',{token_hash:token,type:'email'});assert.equal(verified.status,200,await verified.clone().text());const session=cookie(verified);assert.ok(session.includes('auth-token'));
  ...
  assert.equal((await post('verify',{token_hash:token,type:'email'})).status,400,'Replayed token must fail');
  ...
  console.log('PASS: unconfirmed denial, non-consuming confirmation GETs, cross-browser token verification with no PKCE cookies, signed-in redirects, onboarding gate, replay denial, incorrect-password denial, login, logout cookie clearing, and both local hostnames. No email sent.');
