...
7. Configure **Resend custom SMTP** before student testing using [the Resend setup guide](resend-email-setup.md). Resend delivers the email; Supabase Auth creates and validates verification tokens. `studycircles.me` is verified in Resend; saving SMTP credentials and testing delivery remain pending. Supabase's built-in sender is limited to team-authorized recipients and two emails per hour.
