...
There are 15 local migration files (001–015), including the additional class channels merged from main after this review. Version 004 contains comments only, preserving an empty historical catalog entry; it supplies no catalog SQL. Do not fabricate its contents or infer that all hosted versions are applied. [Team setup](supabase-setup.md) gives the current sequence and replaces obsolete two-migration/auth-only guidance. Earlier feature-doc references to class files/chat files as 004/005 are historical numbering, not deployment instructions for this checkout.
...
> Close the historical folder scope as superseded by #17, not as proof of hosted completion. Old Lectures URLs now redirect to Files, which uses class-wide `kb_uploads`/`kb_assets`, private `class-notes` Storage, and the knowledge worker. Legacy folder generation remains for compatibility. Private uploads, preview/download, uploader retry/removal, and background indexing are implemented locally. The hosted acceptance checklist above remains required under #17, with migration/policy setup tracked by #1. Link this checklist from #17 before closing #6 so no remaining verification is lost. Focused local checks passed (11 tests); the parent also verified 137 offline tests and a production build. Neither result establishes hosted Storage or worker readiness.
